...
model_ = MyGNN(node_input_size=node_featurizer.feat_size(), edge_input_size=edge_featurizer.feat_size(),
               num_gnn_layers=3, num_update_times=1,
               graph_size=105, hidden_size1=60, hidden_size2=11,
...
for i, batch_data in enumerate(pred_loader):
    ...
    med_frags, _ = split_molecules_by_color(smiles, bg, att, threshold=0.5)
    med_frag_list = [Chem.MolToSmiles(mol) for mol in med_frags]

    med_frag_split = med_frag_list[0].split('.')

    # with open('./number.csv', mode='a', newline='') as csvfile:
    #     writer = csv.writer(csvfile)
    #     writer.writerow([len(med_frag_split)])

    filtered_med_frag = [frags_m for frags_m in med_frag_split if len(frags_m) >= 1]

    with open('tmp_2.csv', 'a') as file:
        for med_frag in filtered_med_frag:
            file.write(med_frag + '\n')
